...
*/
//==================================================================================================
#ifndef BOOST_SIMD_ARCH_COMMON_DETAIL_SCALAR_TRIG_REDUCTION_HPP_INCLUDED
#define BOOST_SIMD_ARCH_COMMON_DETAIL_SCALAR_TRIG_REDUCTION_HPP_INCLUDED
...
namespace boost { namespace simd
{
  namespace detail
  {
    ...
    struct trig_reduction < A0, tag::radian_tag, tag::not_simd_type, mode>
    {
      ...
      static BOOST_FORCEINLINE auto is_0_pio4_reduced(A0 a0) BOOST_NOEXCEPT
      ->  decltype(is_not_greater(a0, Pio_4<A0>()))
      {
        return is_not_greater(a0, Pio_4<A0>());
      }
      static BOOST_FORCEINLINE auto is_0_pio2_reduced(A0 a0) BOOST_NOEXCEPT
      ->  decltype(is_not_greater(a0, Pio_2<A0>()))
      {
        return is_not_greater(a0, Pio_2<A0>());
      }
      static BOOST_FORCEINLINE auto is_0_20pi_reduced(A0 a0) BOOST_NOEXCEPT
      ...
      static BOOST_FORCEINLINE auto is_0_mpi_reduced (A0 a0) BOOST_NOEXCEPT
      ->  decltype(is_not_greater(a0, Medium_pi<A0>()))
      {
        return is_not_greater(a0, Medium_pi<A0>()); //2^6 pi
      }
      static BOOST_FORCEINLINE auto is_0_dmpi_reduced(A0 a0) BOOST_NOEXCEPT
      ->  decltype(is_not_greater(a0,Constant<A0,262144>()))
      {
        return is_not_greater(a0, Ratio<A0,262144>()); //2^18 pi
      }

      static BOOST_FORCEINLINE l_t cot_invalid(A0  )  BOOST_NOEXCEPT
      {
        return False<l_t>();
      }
      static BOOST_FORCEINLINE l_t tan_invalid(A0  )  BOOST_NOEXCEPT
      {
        return False<l_t>();
      }
      ...
      struct preliminary
      {
        static BOOST_FORCEINLINE A0 clip(A0  x) BOOST_NOEXCEPT  { return x; }
      };
      ...
      {
        static BOOST_FORCEINLINE A0 clip(A0  x) BOOST_NOEXCEPT
        {
          return clipto(x, boost::mpl::int_<Mode::range>());
        }
      private :
        static BOOST_FORCEINLINE A0 clipto(A0  x
                 ...
                 , boost::mpl::int_<tag::r_0_pio2> const&) BOOST_NOEXCEPT
      {
        ...
        {//to change to rem_pio2_straight call if we want a clipped to pio_2
          xr = xx-Pio2_1<A0>();
          xr -= Pio2_2<A0>();
          xr -= Pio2_3<A0>();
          return One<i_t>();
