...
/**/
  ...
  static std::int32_t __ieee754_rem_pio2f(A0 x, A0 *y)
  {
    /*
    ...
    if(ix<=0x43490f80) { /* |x| ~<= 2^7*(pi/2), medium_ size */
      t  = fabsf(x);
      n  = (std::int32_t) (t*invpio2+Half<A0>());
      fn = (A0)n;
      r  = t-fn*pio2_1;
      w  = fn*pio2_1t;    /* 1st round good to 40 bit */
      if(n<32&&(std::int32_t)(ix&0xffffff00)!=npio2_hw[n-1]) {
    ...
    SET_A0_WORD(z, ix - ((std::int32_t)(e0<<23)));
    for(i=0;i<2;i++) {
      tx[i] = (A0)((std::int32_t)(z));
      z     = (z-tx[i])*two8;
    }
    tx[2] = z;
    nx = 3;
  ...
  static int __kernel_rem_pio2f(A0 *x, A0 *y, int e0, int nx, int prec, const std::int32_t *ipio2)
  {
  ...
  recompute:
    ...
    for(i=0,j=jz,z=q[jz];j>0;i++,j--) {
      fw    =  (A0)((std::int32_t)(twon8* z));
      iq[i] =  (std::int32_t)(z-two8*fw);
      z     =  q[j-1]+fw;
    }

    /* compute n */
    z  = bs::ldexp(z,q0);        /* actual value of z */
    z -= (A0)8.0f*bs::floor(z*(A0)0.125f);    /* trim off integer >= 8 */
    n  = (std::int32_t) z;
    z -= (A0)n;
    ih = 0;
    ...
    if(ih>0) {    /* q > 0.5f */
      ...
      if(ih==2) {
        z = One<A0>() - z;
        if(carry!=0) z -= bs::ldexp(One<A0>(),q0);
      }
    }
    ...
    if(z==(A0)0.0f) {
      ...
      if(z>=two8) {
        fw = (A0)((std::int32_t)(twon8*z));
        iq[jz] = (std::int32_t)(z-two8*fw);
        jz += 1; q0 += 8;
        iq[jz] = (std::int32_t) fw;
      } else iq[jz] = (std::int32_t) z ;
    }

    /* convert integer "bit" chunk to floating-point value */
    fw = bs::ldexp(One<A0>(),q0);
    for(i=jz;i>=0;i--) {
...
/**/
  ...
  static std::int32_t __ieee754_rem_pio2(A0 x, A0 *y)
  {
    static const std::int32_t two_over_pi[] = {
  ...
  static int __kernel_rem_pio2(A0 *x, A0 *y, int e0, int nx, int prec, const std::int32_t *ipio2)
  {
