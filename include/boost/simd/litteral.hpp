...
namespace boost { namespace simd
{
  ...
  namespace detail
  {
    ...
    constexpr std::uint64_t parse(int,const char (&)[N], bm::uint64_t<0> const&)
    {
      return 0;
    }
    ...
    constexpr std::uint64_t parse(int base, const char (&arr)[N], bm::uint64_t<I> const&)
    {
      return to_int(arr[I - 1]) * base + parse<N>(base*10, arr,bm::uint64_t<I-1>{});
    }

    // Intermediate trampoline so MSVC doesn't cry
    template<std::size_t N,char... c> constexpr std::uint64_t parse(int base)
    {
      return parse<N>(base, {c...}, bm::uint64_t<N>{});
    }
  }
  ...
  namespace literal
  {
    ...
    template <char ...c>
    constexpr bm::uint64_t< boost::simd::detail::parse<sizeof...(c),c...>(1)> operator"" _c()
    {
      return {};
    }
  }
