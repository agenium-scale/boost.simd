...
// STF_CASE_TPL( "Check is_gt behavior with exotic type", STF_IEEE_TYPES )
// {
//   namespace bs = boost::simd;
//   using bs::is_gt;
//   using foo::nontrivial;
//   using r_t = decltype(is_gt(nontrivial<T>(), nontrivial<T>()));
//   STF_TYPE_IS(r_t, nontrivial<T>);

//   STF_EQUAL(is_gt(nontrivial<T>(1, 2), nontrivial<T>(3, 4)), nontrivial<T>(4, 8));
// }
