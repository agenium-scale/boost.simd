...
STF_CASE_TPL (" is_ltz  _real",  STF_IEEE_TYPES)
{
  namespace bs = boost::simd;
  namespace bd = boost::dispatch;
  using bs::is_ltz;
  using r_t = decltype(is_ltz(T()));
...
STF_CASE ( "is_ltz _bool")
{
  namespace bs = boost::simd;
  namespace bd = boost::dispatch;
  using bs::is_ltz;
  using r_t = decltype(is_ltz(bool()));

  // return type conformity test
  STF_TYPE_IS(r_t, bool);

  // specific values tests
  STF_EQUAL(is_ltz(true), false);
  STF_EQUAL(is_ltz(false), false);
}
