...
STF_CASE_TPL (" erfc std",  STF_IEEE_TYPES)
{
  ...
  // specific values tests
#ifndef BOOST_SIMD_NO_INVALIDS
  STF_ULP_EQUAL(erfc(bs::Inf<T>(), bs::std_), bs::Zero<r_t>(), 0);
  STF_ULP_EQUAL(erfc(bs::Nan<T>(), bs::std_), bs::Nan<r_t>(), 0);
#endif
  ...
  STF_EXPECT(bs::is_less(erfc(T(15), bs::std_)- T(7.212994172451206e-100), bs::Eps<T>()));
  STF_ULP_EQUAL(erfc(T(-6), bs::std_), T(2), 0);
}
