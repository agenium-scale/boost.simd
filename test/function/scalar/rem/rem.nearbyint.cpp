...
STF_CASE_TPL (" remainder real",  STF_IEEE_TYPES)
{
  ...
  using bs::rem;
  using r_t = decltype(rem(bs::nearbyint, T(), T()));
...
STF_CASE_TPL (" remainder signed_int",  STF_SIGNED_INTEGRAL_TYPES)
{
  ...
  STF_EQUAL(rem(bs::nearbyint, bs::Mone<T>(), bs::Mone<T>()), bs::Zero<T>());
  STF_EQUAL(rem(bs::nearbyint, bs::One<T>(), bs::One<T>()), bs::Zero<T>());
  STF_EQUAL(rem(bs::nearbyint, bs::Zero<T>(), bs::Zero<T>()), bs::Zero<T>());
  STF_EQUAL(rem(bs::nearbyint, bs::Two<T>(), bs::Three<T>()), bs::Mone<T>());
} // end of test for signed_int_


STF_CASE_TPL (" rem nearbyint fast",  STF_IEEE_TYPES)
{
  namespace bs = boost::simd;
  namespace bd = boost::dispatch;
  using bs::rem;
