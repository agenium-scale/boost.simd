...
STF_CASE_TPL (" max real",  STF_IEEE_TYPES)
{
  namespace bs = boost::simd;

  using bs::max;
  // return type conformity test
  STF_EXPR_IS(max(T(),T()), T);
...
STF_CASE_TPL (" max unsigned int",  STF_UNSIGNED_ALL_INTEGRAL_TYPES)
{
  namespace bs = boost::simd;
  ...
  STF_EQUAL(max(bs::One<T>(),  bs::One<T>()),  bs::One<T>());
  STF_EQUAL(max(bs::Zero<T>(), bs::Zero<T>()), bs::Zero<T>());
  STF_EQUAL(max(bs::One<T>(),  bs::Zero<T>()), bs::One<T>());
  STF_EQUAL(max(bs::Zero<T>(), bs::One<T>()),  bs::One<T>());
} // end of test for unsigned_int_

STF_CASE_TPL (" max signed int",  STF_SIGNED_ALL_INTEGRAL_TYPES)
{
  namespace bs = boost::simd;
  ...
  STF_EQUAL(max(bs::One<T>(),  bs::Zero<T>()), bs::One<T>());
  STF_EQUAL(max(bs::Zero<T>(), bs::One<T>()),  bs::One<T>());
  STF_EQUAL(max(bs::Mone<T>(), bs::Zero<T>()), bs::Zero<T>());
  STF_EQUAL(max(bs::Zero<T>(), bs::Mone<T>()), bs::Zero<T>());
