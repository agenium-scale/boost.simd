...
STF_CASE_TPL (" atan2pi",  STF_IEEE_TYPES)
{
  ...
  using r_t = decltype(atan2pi(T(), T()));
  ...
  STF_ULP_EQUAL(atan2pi(bs::Zero<T>(), bs::Nan<T>()), bs::Nan<r_t>(), 0.5);
  STF_ULP_EQUAL(atan2pi(bs::One<T>(),bs::Inf<T>()), bs::Zero<r_t>(), 0.5);
  STF_ULP_EQUAL(atan2pi(bs::One<T>(),bs::Minf<T>()), T(1.0), 0.5);
#endif
  STF_ULP_EQUAL(atan2pi(bs::Half<T>(), bs::Half<T>()), T(0.25), 0.5);
