...
STF_CASE_TPL(" frexp0", STF_IEEE_TYPES)
{
...

  {
    ...
    T  m;
    T  a = bs::Valmax<T>();
    std::tie(m, e) = bs::pedantic_(frexp)(a);
...
STF_CASE_TPL(" bs::pedantic_(frexp)", STF_IEEE_TYPES)
{
  namespace bs = boost::simd;
  namespace bd = boost::dispatch;
  using bs::frexp;
  ...
  STF_EXPR_IS( (bs::pedantic_(frexp)(T()))
             , (std::pair<T,T>)
             );

  {
    ...
    std::tie(m, e) = bs::pedantic_(frexp)(bs::One<T>());
...

  {
    ...
    p = bs::pedantic_(frexp)(bs::One<T>());
    STF_EQUAL(p.first  , bs::Half<T>());
    STF_EQUAL(p.second , bs::One<T>());
  }
}

STF_CASE_TPL(" bs::pedantic_(frexp)0", STF_IEEE_TYPES)
{
  namespace bs = boost::simd;
  namespace bd = boost::dispatch;
  using bs::frexp;
