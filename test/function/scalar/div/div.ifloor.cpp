...
STF_CASE_TPL (" div real",  STF_IEEE_TYPES)
{
  ...
  using bs::div;
  using r_t = decltype(bs::div(bs::ifloor, T(), T()));
...
STF_CASE_TPL (" divunsigned_int",  STF_UNSIGNED_INTEGRAL_TYPES)
{
  ...
  // specific values tests
  STF_EQUAL(div(bs::ifloor, T(4),T(0)), bs::Valmax<r_t>());
  STF_EQUAL(div(bs::ifloor, T(4),T(3)), T(1));
  STF_EQUAL(div(bs::ifloor, bs::One<T>(), bs::One<T>()), bs::One<r_t>());
  STF_EQUAL(div(bs::ifloor, bs::Valmax<T>(),  bs::Two<T>()), bs::Valmax<r_t>()/bs::Two<T>());
...
STF_CASE_TPL (" divsigned_int",  STF_SIGNED_INTEGRAL_TYPES)
{
  ...
  // specific values tests
  STF_EQUAL(div(bs::ifloor, T(-4),T(0)), bs::Valmin<r_t>());
  STF_EQUAL(div(bs::ifloor, T(4),T(0)), bs::Valmax<r_t>());
  STF_EQUAL(div(bs::ifloor, T(4),T(3)), T(1));
  STF_EQUAL(div(bs::ifloor, T(-4),T(-3)), T(1));
  STF_EQUAL(div(bs::ifloor, T(4),T(-3)), T(-2));
  STF_EQUAL(div(bs::ifloor, T(-4),T(3)), T(-2));
  STF_EQUAL(div(bs::ifloor, bs::Mone<T>(), bs::Mone<T>()), bs::One<r_t>());
  STF_EQUAL(div(bs::ifloor, bs::One<T>(), bs::One<T>()), bs::One<r_t>());
