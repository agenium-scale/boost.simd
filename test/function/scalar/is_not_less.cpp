...
STF_CASE_TPL (" is_not_less integer",  STF_INTEGRAL_TYPES)
{
  namespace bs = boost::simd;
  namespace bd = boost::dispatch;
  using bs::is_not_less;
  using r_t = decltype(is_not_less(T(), T()));

  // return type conformity test
  STF_TYPE_IS(r_t, bs::logical<T>);

  // specific values tests
  STF_EQUAL(is_not_less(bs::One<T>(), bs::One<T>()), r_t(true));
  STF_EQUAL(is_not_less(bs::One<T>(),bs::Zero<T>()), r_t(true));
  STF_EQUAL(is_not_less(bs::Zero<T>(), bs::Zero<T>()), r_t(true));
} // end of test for integer_

STF_CASE_TPL (" is_not_less real",  STF_IEEE_TYPES)
{
  namespace bs = boost::simd;
  namespace bd = boost::dispatch;
  using bs::is_not_less;
...
STF_CASE ( "is_not_less bool")
{
  ...
  namespace bd = boost::dispatch;
  using bs::is_not_less;
  using r_t = decltype(is_not_less(bool(), bool()));
