...
STF_CASE_TPL( "Check store behavior with masked pointer with or without offset and mask", STF_NUMERIC_TYPES )
{
  T res(0);
  bs::store(T(42), bs::mask(&res, true));
  STF_EQUAL(res, T(42) );

  bs::store(T(69), bs::mask(&res, false));
  STF_EQUAL(res, T(42) );

  T src1[4] = {T(1), T(2), T(3), T(4)};
  T res1[4] = {T(0), T(0), T(0), T(0)};

  for(int i=0; i < 4 ; ++i) bs::store(src1[i], bs::mask(&res1[0], i%2), i);
