...
void test(Env& runtime)
{
  ...
  auto that = bs::ifrexp(in);

  STF_EQUAL(that.first, mm);
  STF_EQUAL(that.second, ee);
}

STF_CASE_TPL("Check basic behavior of ifrexp on pack" , STF_IEEE_TYPES)
{
...
{
  auto r = bs::ifrexp(bs::Valmax<bs::pack<T>>());
