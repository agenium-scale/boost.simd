...
void test(Env& runtime)
{
  ...
  STF_IEEE_EQUAL(bs::bitwise_or(aa1, aa2), bb);
  STF_IEEE_EQUAL(aa1 | aa2, bb);
}

STF_CASE_TPL("Check bitwise_or on pack" , STF_NUMERIC_TYPES)
{
...
void testm(Env& runtime)
{
  ...
  for(std::size_t i = 0; i < N; ++i)
  {
     b1[i] = bs::bitwise_or(a1[i], a2[i]);
     b2[i] = bs::bitwise_or(a2[i], a1[i]);
     b3[i] = bs::bitwise_or(a1[i], a3[i]);
     b4[i] = bs::bitwise_or(a3[i], a1[i]);
  }
  ...
  STF_IEEE_EQUAL(bs::bitwise_or(aa3, aa1) , bb4);
  STF_IEEE_EQUAL(aa3 | aa1                , bb4);
}

STF_CASE_TPL("Check bitwise_or on pack mixed" , STF_IEEE_TYPES)
{
  static const std::size_t N = bs::pack<T>::static_size;
