...
void test(Env& $)
{
  ...
  std::tie(mm1, ee1) = bs::pedantic_(bs::ifrexp)(aa1);
...
STF_CASE_TPL(" bs::pedantic_(ifrexp)", STF_IEEE_TYPES)
{
  ...
  STF_EXPR_IS( (bs::pedantic_(ifrexp)(p_t()))
             , (std::pair<p_t,pi_t>)
             );

  {
    ...
    std::tie(m, e) = bs::pedantic_(ifrexp)(bs::One<p_t>());
...

  {
    ...
    p = bs::pedantic_(ifrexp)(bs::One<p_t>());
