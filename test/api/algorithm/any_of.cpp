...
STF_CASE_TPL( "Check simd::any_of(f,l)", STF_NUMERIC_TYPES )
{
...

  {
    ...
    // All aligned
    STF_EQUAL ( false
              , (boost::simd::any_of(ab,ae))
              );

    // prologue + aligned
    STF_EQUAL ( false
              , (boost::simd::any_of(ab+1,ae))
              );

    // aligned + epilogue
    STF_EQUAL ( false
              , (boost::simd::any_of(ab,ae-1))
              );

    // prologue + epilogue
    STF_EQUAL ( false
              , (boost::simd::any_of(ab+1,ae-1))
              );
  }
  {
