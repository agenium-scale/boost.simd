...
STF_CASE_TPL( "Check unary simd::find_if_not", STF_NUMERIC_TYPES )
{
  ...
  {
    auto f1 = std::find_if_not(values.begin(), values.end(), f<T>(N));
    auto f2 = boost::simd::find_if_not(values.data(), values.data()+2*N+1, f<T>(N));
    STF_EQUAL ( *f1, *f2 );
    auto f3 = std::find_if_not(values.begin(), values.end(), f<T>(104));
    auto f4 = boost::simd::find_if_not(values.data(), values.data()+2*N+1, f<T>(104));
    STF_EQUAL ( *f3, *f4 );
  }
  {
    auto f1 = std::find_if_not(values.begin(), values.end(), f<T>(0));
    auto f2 = boost::simd::find_if_not(values.data(), values.data()+2*N+1, f<T>(0));
    STF_EQUAL ( *f1, *f2 );
    auto f3 = std::find_if_not(values.begin(), values.end(), f<T>(2*N));
