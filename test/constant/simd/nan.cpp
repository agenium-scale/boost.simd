...
STF_CASE_TPL( "Check Nan behavior for integrals"
            ...
            )
{
  ...
  auto are_correct = [](T e) {  return e == bs::Zero<T>(); };

  auto pack  = bs::Nan<pack_t>();
  auto hpack = bs::Nan<hpack_t>();
  auto dpack = bs::Nan<dpack_t>();
...
STF_CASE_TPL( "Check Nan behavior for floating points"
            , (double)(float)
             )
{
  namespace bs =  boost::simd;
