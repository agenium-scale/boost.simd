...
template <typename T>
struct is_not_greater_equal_scalar
{
   template <typename U>
   void operator()(U min0, U max0, U min1, U max1)
   {
     ...
     nsb::make_function_experiment_cpe_sized_<1>
       ( [](const T & x0, const T & x1) -> ret_type
         { return bs::is_not_greater_equal(x0, x1); }
...
int main(int argc, char **argv) {
   ...
   nsb::make_for_each<is_not_greater_equal_scalar, NS_BENCH_SIGNED_NUMERIC_TYPES>( -10,  10,  -10,  10);
   nsb::make_for_each<is_not_greater_equal_scalar, NS_BENCH_UNSIGNED_NUMERIC_TYPES>(0,  10, 0,  10);
   return 0;
}
