...
template<typename Value> Value scaldot(Value* first1, Value* last1, Value* first2)
{
  Value v(0);
...
int main()
{
  ...
  std::cout << "scalar dot product output " << scaldot(v1.data(), v1.data()+size, v2.data()) << std::endl;
  std::cout << "simd   dot product output " << simddot(v1.data(), v1.data()+size, v2.data()) << std::endl;

};
// This code can be compiled using (for instance for gcc)
// g++ dotmain.cpp -msse4.2 -std=c++11 -O3 -DNDEBUG -o dotmain -I/path_to/boost_simd/ -I/path_to/boost/
//! [dot-main]
